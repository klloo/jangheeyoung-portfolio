    ...
    const projectList = [
      {
        ...
        troubleShooting: {
          trouble:
            "깃허브의 <b>'잔디 심기'와 유사한 기능을 개발</b>해야 했는데 참고할만한 자료가 없었습니다.",
          process:
            '처음에는 참고할만한 코드나 라이브러리가 있을 것으로 예상했지만 해당 기능의 정확한 명칭도 알 수가 없었고 참고할 만한 내용 또한 찾을 수 없었습니다.<br/>' +
            '따라서 <b>직접 분석 후 개발</b>을 진행하기로 결정하였고, 깃허브 외에 해당 기능을 제공하는 웹사이트 2곳의 HTML 코드를 개발자 도구를 통해 조사했습니다. <br/>' +
            '각 칸을 생성하는데 있어서 한 곳은 table 태그를 사용하고 있었고, 다른 두 곳은 svg 태그를 활용하고 있었습니다.',
          result:
            ...
            "이러한 방식을 통해 '잔디 심기' 기능을 구현할 수 있었고, 처음 개발하는 기능이었지만 참고할만한 코드가 없는 상황에서의 문제 해결 방식을 익힐 수 있는 기회를 얻을 수 있었습니다.",
      ...
      },
      {
        ...
        troubleShooting: {
          ...
          process:
            '각 토큰의 저장 위치를 결정할 때 주요 고려사항은 <b>보안</b>이었습니다.<br/>Access 토큰의 경우, <b>로컬 변수</b>에 저장하면 보안성은 높아지지만 페이지를 새로고침 하거나 새 창에서 접속할 때마다 토큰을 재발급해야 한다는 단점이 있습니다.<br/>' +
          ...
          result:
            ...
            '로컬 변수에 저장할 경우 새로고침을 할 때 마다 재발급 요청을 보내는 것이 비효율적이라고 생각했고 XSS 취약점은 어느 저장 방식을 선택하든 고려를 해야할 문제이기 때문에 조금 더 효율적이고 간단하게 구현할 수 있는 방식을 선택하게 되었습니다.<br/>' +
            '<b>Refresh 토큰</b>은 Access 토큰의 재발급을 위한 것으로 용도가 한정되어 있으며, CSRF 공격으로는 새로 발급받은 엑세스 토큰을 수령할 방법이 없기 때문에 쿠키에 저장해도 CSRF 공격으로 부터 안전합니다.<br/>따라서 자바스크립트 코드로 접근이 불가능한 <b>httpOnly 쿠키</b>로 저장하여 XSS 공격도 방지하는 것이 최선이라고 판단하였습니다. <br/>',
          resultImage: null,
        },
      },
      {
        ...
        troubleShooting: {
          ...
          process:
            '이 프로젝트에서는 axios 요청을 전송하는 Promise들을 따로 관리하고 있었고 API 요청에 대한 응답의 형태가 일관되어있습니다.<br/>' +
          ...
          result:
            '요청에 대한 응답을 확인하는 중복 코드를 매번 작성하지 않아도 되었고, 모든 응답 값 확인이 커스텀 훅에서 이루어지기 때문에 나중에 응답 형태가 변경되더라도 쉽게 대응할 수 있는 구조로 유지보수 또한 간편해졌습니다.',
          resultImage: null,
        },
      },
      {
        ...
        troubleShooting: {
          trouble:
            '포트폴리오를 작성하는 과정에서 중복되는 형식이 많았고, 포트폴리오의 특성 상 빈번한 수정이 필요할 것으로 예상되어 <b>생산성과 유지보수 측면을 고려</b>해야 했습니다.',
          process:
            '중복 코드를 피하고 생산성을 높이기 위해 리액트 환경과 비슷하게 컴포넌트 기반으로 개발하는 것이 좋겠다고 판단했습니다.<br/>' +
            '이에 따라 구글링의 도움으로 <b>클래스형 컴포넌트를 구현</b>하고, 코드를 모듈화하여 컴포넌트를 분할했습니다.<br/>' +
            '그 후에는 Webpack 설정을 통해 모듈화된 코드를 번들링하고, Webpack dev server 설정으로 개발 환경에서도 번들링된 결과물을 확인할 수 있도록 했습니다.',
          result:
            '중복되는 코드들을 컴포넌트로 분리하여 개발 생산성을 향상시키면서 유지보수성도 높일 수 있었습니다.<br/>' +
            '컴포넌트의 상태가 변경되면 해당 컴포넌트를 다시 렌더링하는 과정을 직접 구현하면서, 리액트의 useState 훅과 클래스형 컴포넌트의 생명주기에 대해 더 깊게 이해할 수 있었습니다.',
          resultImage: null,
        },
      },
      {
